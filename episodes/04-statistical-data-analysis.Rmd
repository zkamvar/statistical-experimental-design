...
lowHR <- rnorm(n = 522, mean = 68.6, sd = 10)
medHR <- rnorm(n = 522, mean = 70, sd = 10)
highHR <- rnorm(n = 522, mean = 71.4, sd = 10)
...
HI <- filter(exercise_group == "high intensity")
control <- filter(exercise_group == "control")
...
HI100 <- HI %>% sample(size = 100) %>% mean()
control100 <- control %>% sample(size = 100) %>% mean()
control100 - HI100
```
...
population <- cbind(HI, control)

# sample 100 of them and calculate the mean three times
population %>% sample(size = 100) %>% mean()
population %>% sample(size = 100) %>% mean()
population %>% sample(size = 100) %>% mean()
```
...
population %>% t.test(heart_rate ~ exercise_group)
```
...
Approximately `r round(mean(null >= meanDiff) * 100)`% of the 10,000 simulations 
are greater than the observed difference in means. We can expect then that we 
will see a difference in means approximately 
`r round(mean(null >= meanDiff) * 100)`% of the time even if there is no effect 

...
`r  mean(control$heart_rate) - mean(HI$heart_rate)`. Another way of stating this 
...
(~`r round((sum(population$heart_rate > 90)/dim(population)[1]) * 100)`%) 
with a resting heart rate greater than 90, and another 
`r sum(population$heart_rate < 50)` individuals
(~`r round((sum(population$heart_rate < 50)/dim(population)[1]) * 100)`%) 

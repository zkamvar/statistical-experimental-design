...
```{r, simulate_HR_data, include=FALSE}
# simulate exercise group heart rate data
lowHR <- rnorm(n = 522, mean = 60, sd = 10)
medHR <- rnorm(n = 522, mean = 70, sd = 10)
highHR <- rnorm(n = 523, mean = 80, sd = 10)
lowHR <- cbind(lowHR, rep("high intensity", length(lowHR)))
medHR <- cbind(medHR, rep("moderate intensity", length(medHR)))
highHR <- cbind(highHR, rep("control", length(highHR)))
heart_rate <- as.data.frame(rbind(lowHR, medHR, highHR))
names(heart_rate) <- c("heart_rate", "exercise_group")
heart_rate$heart_rate <- as.numeric(heart_rate$heart_rate)
write.csv(heart_rate, file = "../data/heart_rate.csv")
```
...
mean(control) - mean(HI)
```
The actual difference in mean heart rates is 
`r mean(control) - mean(HI)`. Another way of stating this is that the 
high-intensity group had a mean heart rate that was 
`r round(((mean(control) - mean(HI))/mean(control)) * 100)` percent lower than 
the control group.
...
```{r, population}
# read in the population of all elderly people
HI100 <- mean(sample(HI, size=100)) 
control100 <- mean(sample(control, size=100)) 
control100 - HI100
```
...
`r t.test(heart_rate ~ exercise_group, data = heart_rate)$estimate[[1]]`, which
is lower than the mean heart rate
(`r `t.test(heart_rate ~ exercise_group, data = heart_rate)$estimate[[2]]`)
of people in the control exercise group (p-value = 
`r signif(t.test(heart_rate ~ exercise_group, data = heart_rate)$p.value, 3)`).
...
between `r t.test(heart_rate ~ exercise_group, data = heart_rate)$conf.int[1]` 
and `r t.test(heart_rate ~ exercise_group, data = heart_rate)$conf.int[2]`. 
